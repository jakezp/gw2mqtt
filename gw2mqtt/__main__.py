...
def mqtt_server_connection(mqtt_host, mqtt_subscription_topic):
    client = mqtt.Client("goodwe-test")
...
def goodwe_inverter_connection(gw_inverter_ip, gw_inverter_port):
    try:
...
def run_once(settings):
    ...
    inverter = goodwe_inverter_connection(settings.gw_inverter_ip, settings.gw_inverter_port)
...
def run():
    ...
    parser.add_argument("--gw-interval", help="Interval to collect data from inverter", type=int, choices=[10, 20, 30])
